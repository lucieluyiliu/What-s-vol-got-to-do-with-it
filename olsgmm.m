 function [bv,sebv,R2v,t,R2vadj,v,F,errv,Sigma] = olsgmm(lhv,rhv,lags,weight);
...
% Output:
%  b: regression coefficients K x 1 vector of coefficients(output is a volum vector)
    ...
    for indx = 1:N;
       err=errv(:,indx);
        ...
        if rhv(:,1) == ones(size(rhv,1),1); 
            ...
            dof = size(bv(:,1),1);  %dof is the number of regressors except for the constant
            pval = 1-cdf('chi2',chi2val, dof); 
            F(indx,1:3) = [chi2val dof pval]; 
        end; 
